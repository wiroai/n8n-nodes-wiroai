...
export class FaceTransferSdxlLora implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Face Transfer Sdxl Lora',
		name: 'faceTransferSdxlLora',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Uses SDXL with LoRA to take a photo and generate new photos based on prompts, incorporating fac',
		defaults: {
			name: 'Wiro - Face Transfer Sdxl Lora',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/face-transfer-sdxl-lora',
			headers: {
				...headers,
