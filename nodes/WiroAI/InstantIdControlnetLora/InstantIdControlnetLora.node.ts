...
export class InstantIdControlnetLora implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Instant Id Controlnet Lora',
		name: 'instantIdControlnetLora',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'InstantID is a method to achieve ID-preserving generation with only single image. Alternatively',
		defaults: {
			name: 'Wiro - Instant Id Controlnet Lora',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/instant-id-controlnet-lora',
			headers: {
				...headers,
