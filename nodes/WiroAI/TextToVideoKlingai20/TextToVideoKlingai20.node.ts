...
export class TextToVideoKlingai20 implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Text To Video Klingai V2 0',
		name: 'textToVideoKlingai20',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'KlingAI v2.0 text to video generation tool. Generate 5s and 10s videos in 720p resolution at 30',
		defaults: {
			name: 'Wiro - Text To Video Klingai V2 0',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/klingai/text-to-video-klingai-v2.0',
			headers: {
				...headers,
