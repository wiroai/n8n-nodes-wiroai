...
export class FaceTransferSd implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Face Transfer Sd',
		name: 'faceTransferSd',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'A stable diffusion tool that generates new images based on prompts from a given photo, preservi',
		defaults: {
			name: 'Wiro - Face Transfer Sd',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/face-transfer-sd',
			headers: {
				...headers,
