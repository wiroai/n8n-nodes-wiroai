...
export class GlmImage implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Glm Image',
		name: 'glmImage',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Access GLM Image API by ZAI on Wiro. Simple integration, transparent pricing, and comprehensive',
		defaults: {
			name: 'Wiro - Glm Image',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/zai-org/GLM-IMAGE',
			headers: {
				...headers,
