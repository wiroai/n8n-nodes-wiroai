...
export class ImageToImageSdxl implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Image To Image Sdxl',
		name: 'imageToImageSdxl',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'An image-to-image model that re-generates images to AI art images with prompt',
		defaults: {
			name: 'Wiro - Image To Image Sdxl',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/image-to-image-sdxl',
			headers: {
				...headers,
