...
export class AiExitInterviewEvaluator implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Ai Exit Interview Evaluator',
		name: 'aiExitInterviewEvaluator',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Wiro/AI-Exit-Interview-Evaluator enables you to analyze and evaluate completed exit interview r',
		defaults: {
			name: 'Wiro - Ai Exit Interview Evaluator',
		},
		...
		properties: [
			{
				...
				type: 'string',
				default: '',
				description: 'Upload exit interview forms. Ensure the files are relevant and properly formatted for accurate processing. Supported file types: .csv, .docx, .epub, .jpeg, .jpg, .mbox, .md, .mp3, .mp4, .pdf, .png, .ppt, .pptm, .pptx',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/AI-Exit-Interview-Evaluator',
			headers: {
				...headers,
