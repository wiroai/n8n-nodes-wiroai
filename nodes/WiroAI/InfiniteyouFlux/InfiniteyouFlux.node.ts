...
export class InfiniteyouFlux implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Infiniteyou Flux',
		name: 'infiniteyouFlux',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'InfiniteYou-Flux unlocks the power of ByteDance’s InfiniteYou model, optimized with Wiro’s Flux',
		defaults: {
			name: 'Wiro - Infiniteyou Flux',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/InfiniteYou-flux',
			headers: {
				...headers,
