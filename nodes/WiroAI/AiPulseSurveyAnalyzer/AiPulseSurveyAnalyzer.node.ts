...
export class AiPulseSurveyAnalyzer implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Ai Pulse Survey Analyzer',
		name: 'aiPulseSurveyAnalyzer',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Wiro/AI-Pulse-Survey-Analyzer enables you to analyze and evaluate employee pulse survey respons',
		defaults: {
			name: 'Wiro - Ai Pulse Survey Analyzer',
		},
		...
		properties: [
			{
				...
				type: 'string',
				default: '',
				description: 'Upload employee surveys. Ensure the files are relevant and properly formatted for accurate processing. Supported file types: .csv, .docx, .epub, .jpeg, .jpg, .mbox, .md, .mp3, .mp4, .pdf, .png, .ppt, .pptm, .pptx',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/AI-Pulse-Survey-Analyzer',
			headers: {
				...headers,
