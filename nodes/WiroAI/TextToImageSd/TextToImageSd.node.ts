...
export class TextToImageSd implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Text To Image Sd',
		name: 'textToImageSd',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'A text-to-image tool capable of generating images using a diffusion model',
		defaults: {
			name: 'Wiro - Text To Image Sd',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/text-to-image-sd',
			headers: {
				...headers,
