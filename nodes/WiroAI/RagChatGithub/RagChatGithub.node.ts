...
export class RagChatGithub implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Rag Chat Github',
		name: 'ragChatGithub',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Instantly retrieve and analyze content from any GitHub repository. Select your LLM model, extra',
		defaults: {
			name: 'Wiro - Rag Chat Github',
		},
		...
		properties: [
			...
			{
				displayName: 'Github Repo Owner Name',
				name: 'github_owner',
				type: 'string',
				default: '',
				description: 'Enter a github respository owner name https://github.com/𝐦𝐞𝐭𝐚-𝐥𝐥𝐚𝐦𝐚/llama',
			},
			{
				...
				type: 'string',
				default: '',
				description: 'Enter a github respository name https://github.com/meta-llama/𝐥𝐥𝐚𝐦𝐚',
			},
			{
				...
				type: 'string',
				default: '',
				description: 'Enter a github respository branch name',
			},
			{
				...
				default: '',
				description: 'Provide your GitHub access token for private repositories. For public ones, leave it blank.',
			},
			{
				displayName: 'Prompt',
				name: 'prompt',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const github_owner = this.getNodeParameter('github_owner', 0, '') as string;
		const github_repo = this.getNodeParameter('github_repo', 0, '') as string;
		const github_branch = this.getNodeParameter('github_branch', 0, '') as string;
		const github_token = this.getNodeParameter('github_token', 0, '') as string;
