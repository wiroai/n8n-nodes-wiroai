...
export class KolorsIpAdapter implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Kolors Ip Adapter',
		name: 'kolorsIpAdapter',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Kolors IP-Adapter Tool',
		defaults: {
			name: 'Wiro - Kolors Ip Adapter',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/kolors-IP-Adapter',
			headers: {
				...headers,
