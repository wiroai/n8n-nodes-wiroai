...
export class RagChat implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Rag Chat',
		name: 'ragChat',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Wiro LLM Conversational RAG Tool – A unified interface for seamless RAG-based conversations. Se',
		defaults: {
			name: 'Wiro - Rag Chat',
		},
		...
		properties: [
			...
			},
			{
				...
				type: 'string',
				default: '',
				description: 'Upload multiple document files. Ensure the files are relevant and properly formatted for accurate processing. Supported file types: .csv, .docx, .epub, .jpeg, .jpg, .mbox, .md, .mp3,.mp4, .pdf, .png, .ppt, .pptm, .pptx',
			},
			{
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/rag-chat',
			headers: {
				...headers,
