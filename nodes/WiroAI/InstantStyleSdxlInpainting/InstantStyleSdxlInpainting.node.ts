...
export class InstantStyleSdxlInpainting implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Instant Style Sdxl Inpainting',
		name: 'instantStyleSdxlInpainting',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Image to image model that applies the style design of the given image to the prompt',
		defaults: {
			name: 'Wiro - Instant Style Sdxl Inpainting',
		},
		...
		properties: [
			...
			{
				displayName: 'Init Image Mask URL',
				name: 'inputImageMaskUrl',
				type: 'string',
				default: '',
				description: 'Init-image-mask-help',
			},
			{
				displayName: 'Prompt',
				name: 'prompt',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/instant_style_sdxl_inpainting',
			headers: {
				...headers,
