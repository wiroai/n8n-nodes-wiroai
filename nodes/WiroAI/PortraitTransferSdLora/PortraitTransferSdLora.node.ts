...
export class PortraitTransferSdLora implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Portrait Transfer Sd Lora',
		name: 'portraitTransferSdLora',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Employs LoRA with stable diffusion to generate new portrait images from a photo based on prompt',
		defaults: {
			name: 'Wiro - Portrait Transfer Sd Lora',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/portrait-transfer-sd-lora',
			headers: {
				...headers,
