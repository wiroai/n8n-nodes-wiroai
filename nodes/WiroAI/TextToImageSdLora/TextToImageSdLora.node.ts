...
export class TextToImageSdLora implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Text To Image Sd Lora',
		name: 'textToImageSdLora',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'A text-to-image tool capable of generating images using a Stable Diffusion model and LoRA',
		defaults: {
			name: 'Wiro - Text To Image Sd Lora',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const clipSkip = this.getNodeParameter('clipSkip', 0, 0) as number;
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/text-to-image-sd-lora',
			headers: {
				...headers,
