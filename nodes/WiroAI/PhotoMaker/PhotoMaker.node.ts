...
export class PhotoMaker implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Photo Maker',
		name: 'photoMaker',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'It applies the average of the input images to the given prompt',
		defaults: {
			name: 'Wiro - Photo Maker',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/photo-maker',
			headers: {
				...headers,
