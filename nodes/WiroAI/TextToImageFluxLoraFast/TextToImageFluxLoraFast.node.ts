...
export class TextToImageFluxLoraFast implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Text To Image Flux Lora Fast',
		name: 'textToImageFluxLoraFast',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Flux is a tool that generates images from text descriptions. With LoRA models, you can enhance',
		defaults: {
			name: 'Wiro - Text To Image Flux Lora Fast',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/text-to-image-flux-lora-fast',
			headers: {
				...headers,
