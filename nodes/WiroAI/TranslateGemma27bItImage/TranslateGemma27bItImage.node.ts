...
export class TranslateGemma27bItImage implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Translate Gemma 27B It Image',
		name: 'translateGemma27bItImage',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'TranslateGemma is a family of lightweight, state-of-the-art open translation models from Google',
		defaults: {
			name: 'Wiro - Translate Gemma 27B It Image',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/google/translate-gemma-27b-it-image',
			headers: {
				...headers,
