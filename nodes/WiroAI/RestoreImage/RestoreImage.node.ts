...
export class RestoreImage implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Restore Image',
		name: 'restoreImage',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'An image-to-image AI model engineered for robust image restoration, effectively repairing and e',
		defaults: {
			name: 'Wiro - Restore Image',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/restore-image',
			headers: {
				...headers,
