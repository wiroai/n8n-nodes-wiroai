...
export class ControlnetInpaintingSd implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Controlnet Inpainting Sd',
		name: 'controlnetInpaintingSd',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'ControlNet is a neural network structure to control diffusion models by adding extra conditions',
		defaults: {
			name: 'Wiro - Controlnet Inpainting Sd',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/controlnet-inpainting-sd',
			headers: {
				...headers,
