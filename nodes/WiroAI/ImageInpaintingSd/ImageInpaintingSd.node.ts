...
export class ImageInpaintingSd implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Image Inpainting Sd',
		name: 'imageInpaintingSd',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Transform your image editing experience with our AI generative model-based image inpainting sol',
		defaults: {
			name: 'Wiro - Image Inpainting Sd',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/image-inpainting-sd',
			headers: {
				...headers,
