...
export class DepthToImageSd implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Depth To Image Sd',
		name: 'depthToImageSd',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Create variations of an image while preserving shape and depth',
		defaults: {
			name: 'Wiro - Depth To Image Sd',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/depth-to-image-sd',
			headers: {
				...headers,
