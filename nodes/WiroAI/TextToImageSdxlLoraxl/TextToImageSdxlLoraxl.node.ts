...
export class TextToImageSdxlLoraxl implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Text To Image Sdxl Loraxl',
		name: 'textToImageSdxlLoraxl',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'A text-to-image tool capable of generating images using a Stable Diffusion XL model and LoRA XL',
		defaults: {
			name: 'Wiro - Text To Image Sdxl Loraxl',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/text-to-image-sdxl-loraxl',
			headers: {
				...headers,
