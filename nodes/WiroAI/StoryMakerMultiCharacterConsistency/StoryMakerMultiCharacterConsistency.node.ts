...
export class StoryMakerMultiCharacterConsistency implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Story Maker Multi Character Consistency',
		name: 'storyMakerMultiCharacterConsistency',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Maintains character consistency by preserving the people in the two images it receives and appl',
		defaults: {
			name: 'Wiro - Story Maker Multi Character Consistency',
		},
		...
		properties: [
			...
			{
				displayName: 'Input Image 2 URL',
				name: 'inputImage2Url',
				type: 'string',
				default: '',
				description: 'Init-image-2-help',
			},
			{
				displayName: 'Prompt',
				name: 'prompt',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/story-maker-multi-character-consistency',
			headers: {
				...headers,
