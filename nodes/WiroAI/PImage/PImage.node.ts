...
export class PImage implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - P Image',
		name: 'pImage',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'P-Image by Pruna-AI offers a fast text-to-image API with transparent pricing and image samples',
		defaults: {
			name: 'Wiro - P Image',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/pruna/p-image',
			headers: {
				...headers,
