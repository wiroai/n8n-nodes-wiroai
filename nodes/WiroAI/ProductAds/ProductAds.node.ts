...
export class ProductAds implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Product Ads',
		name: 'productAds',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'AI-powered Product Ads API transforms static product images into captivating animated video ads',
		defaults: {
			name: 'Wiro - Product Ads',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/Product Ads',
			headers: {
				...headers,
