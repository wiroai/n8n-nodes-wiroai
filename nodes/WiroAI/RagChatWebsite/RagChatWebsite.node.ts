...
export class RagChatWebsite implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Rag Chat Website',
		name: 'ragChatWebsite',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Instantly retrieve and analyze content from any website URL. Select your LLM model, fetch key i',
		defaults: {
			name: 'Wiro - Rag Chat Website',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/rag-chat-website',
			headers: {
				...headers,
