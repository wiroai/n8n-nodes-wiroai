...
export class WhisperLarge3TurboTurkish implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Speech to Text',
		name: 'whisperLarge3TurboTurkish',
		...
		properties: [
			...
			},
			{
				...
				default: 0,
				required: true,
				description: 'Number of items processed at once',
			},
			{
			...
			},
			{
				...
				type: 'boolean',
				default: false,
				description: 'Whether to enable separate speech by different speakers',
			},
		],
	};
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/openai/whisper-large-v3-turbo-turkish',
			headers: {
				...headers,
