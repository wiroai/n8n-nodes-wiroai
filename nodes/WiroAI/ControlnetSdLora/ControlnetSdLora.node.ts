...
export class ControlnetSdLora implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Controlnet Sd Lora',
		name: 'controlnetSdLora',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'ControlNet is a neural network structure to control diffusion models by adding extra conditions',
		defaults: {
			name: 'Wiro - Controlnet Sd Lora',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/controlnet-sd-lora',
			headers: {
				...headers,
