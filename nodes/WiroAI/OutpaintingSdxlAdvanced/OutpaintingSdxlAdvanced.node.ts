...
export class OutpaintingSdxlAdvanced implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Outpainting Sdxl Advanced',
		name: 'outpaintingSdxlAdvanced',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Outpainting is the process to extend beyond the boundaries of an existing image canvas',
		defaults: {
			name: 'Wiro - Outpainting Sdxl Advanced',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/outpainting-sdxl-advanced',
			headers: {
				...headers,
