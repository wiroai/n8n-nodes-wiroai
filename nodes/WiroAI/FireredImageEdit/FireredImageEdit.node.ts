...
export class FireredImageEdit implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - FireRed Image Edit',
		name: 'fireredImageEdit',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'FireRed Image Edit tool for AI image generation. Edit & create images with text prompts. Perfec',
		defaults: {
			name: 'Wiro - FireRed Image Edit',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/FireRedTeam/FireRed-Image-Edit',
			headers: {
				...headers,
