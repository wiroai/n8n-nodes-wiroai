...
export class InstantStyleSdxlControlnet implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Instant Style Sdxl Controlnet',
		name: 'instantStyleSdxlControlnet',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Image to image model that applies the style design of the given image to the prompt',
		defaults: {
			name: 'Wiro - Instant Style Sdxl Controlnet',
		},
		...
		properties: [
			...
			{
				displayName: 'Controlnet Condition Scale',
				name: 'controlnetConditionScale',
				type: 'number',
				default: 0,
				description: 'Guidancescale-help',
			},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const controlnetConditionScale = this.getNodeParameter('controlnetConditionScale', 0, 0) as number;
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/instant_style_sdxl_controlnet',
			headers: {
				...headers,
