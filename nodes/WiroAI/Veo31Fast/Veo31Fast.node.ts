...
export class Veo31Fast implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Veo 3.1 Fast',
		name: 'veo31Fast',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Create high-fidelity 720p/1080p videos with audio using the Veo 3.1 Fast API. Fast integration,',
		defaults: {
			name: 'Wiro - Veo 3.1 Fast',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/google/veo3.1-fast',
			headers: {
				...headers,
