...
export class StoryDiffusionAdvanced implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Story Diffusion Advanced',
		name: 'storyDiffusionAdvanced',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'It generates comic book-style images of the entered storylines in sequence. Additionally, we ca',
		defaults: {
			name: 'Wiro - Story Diffusion Advanced',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/story-diffusion-advanced',
			headers: {
				...headers,
