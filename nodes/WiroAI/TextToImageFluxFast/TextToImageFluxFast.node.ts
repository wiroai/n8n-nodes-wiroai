...
export class TextToImageFluxFast implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Text To Image Flux Fast',
		name: 'textToImageFluxFast',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Flux Fast is a tool that generates images from text descriptions.',
		defaults: {
			name: 'Wiro - Text To Image Flux Fast',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/text-to-image-flux-fast',
			headers: {
				...headers,
