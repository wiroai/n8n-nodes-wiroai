...
export class Hailuo23 implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - MiniMax Hailuo 2.3',
		name: 'hailuo23',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Experience the MiniMax Hailuo 2.3 API — optimized for cinematic precision, emotional depth, and',
		defaults: {
			name: 'Wiro - MiniMax Hailuo 2.3',
		},
		...
		properties: [
			{
				...
				type: 'string',
				default: '',
				description: 'Optional. First frame image for video generation. The output video will have the same aspect ratio as this image',
			},
			{
				displayName: 'Prompt',
				name: 'prompt',
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const headers = generateWiroAuthHeaders(apiKey, apiSecret);
			...
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/MiniMax/hailuo-2.3',
			headers: {
				...headers,
