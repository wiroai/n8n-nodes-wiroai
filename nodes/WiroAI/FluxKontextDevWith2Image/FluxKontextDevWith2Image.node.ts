...
export class FluxKontextDevWith2Image implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Flux Kontext Dev With 2 Image',
		name: 'fluxKontextDevWith2Image',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'Flux Kontext is an AI tool that allows you to edit images using simple text instructions. Descr',
		defaults: {
			name: 'Wiro - Flux Kontext Dev With 2 Image',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/wiro/flux_kontext_dev_with_2_image',
			headers: {
				...headers,
