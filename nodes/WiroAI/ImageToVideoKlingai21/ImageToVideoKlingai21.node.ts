...
export class ImageToVideoKlingai21 implements INodeType {
	description: INodeTypeDescription = {
		displayName: 'Wiro - Image To Video Klingai V2 1',
		name: 'imageToVideoKlingai21',
		icon: { light: 'file:wiro.svg', dark: 'file:wiro.svg' },
		group: ['transform'],
		version: 1,
		description: 'KlingAI v2.1 image to video generation tool. Generate 5s and 10s videos in 720p resolution at 3',
		defaults: {
			name: 'Wiro - Image To Video Klingai V2 1',
		},
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		const response = await this.helpers.request({
			method: 'POST',
			url: 'https://api.wiro.ai/v1/Run/klingai/image-to-video-klingai-v2.1',
			headers: {
				...headers,
